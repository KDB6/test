...
router.beforeEach(( to:Partial<RouteInfo>, from: Partial<RouteInfo> , next) => {
  ...
  //  '||'와 유사한데  0, false 값도 undefined, null 과 동일하게 처리되어서 ?? 사용을 권장한다.
  // 아래 같은경우는 to.meta.requiresAuth 값이 있으면 이 값을 사용하고 만약 to 또는 to.meta값이 undefined이면 오류 안던지고 undefined를 리턴하는데 최종적으로는  undefined ?? true 이런 코드가 되어 true값이 된다
